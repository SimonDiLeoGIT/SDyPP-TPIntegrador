...
def find_nonce_with_prefix(target_hash_prefix, base_string, start_nonce, end_nonce):
    """
    Encuentra un nonce tal que el hash MD5 del (base_string + nonce) comience con target_hash_prefix.
...
def main(target_hash_prefix, base_string, start_nonce, end_nonce):
    result = find_nonce_with_prefix(target_hash_prefix, base_string, start_nonce, end_nonce)
